...
pub enum AlertType {
    ...
    DailyLossLimit { bot_id: String, current_loss: Decimal, limit: Decimal },
    MaxDrawdown { bot_id: String, current_dd: Decimal, limit: Decimal },
    PositionSize { bot_id: String, current_pct: Decimal, limit: Decimal },
    
    /// System health alerts
    ProvisionFailure { bot_id: String, attempt: u32 },
    OrphanedBot { bot_id: String, status: String, duration_secs: u64 },
    HighErrorRate { component: String, error_rate: f64, threshold: f64 },
...
impl AlertManager {
    ...
    pub async fn check_trading_metrics(
        ...
        if daily_pnl < Decimal::ZERO && daily_pnl.abs() >= self.config.daily_loss_threshold_pct {
            ...
            if self.should_fire(&key, 3600).await { // 1 hour cooldown
        ...
        if max_drawdown >= self.config.max_drawdown_threshold_pct {
            ...
            if self.should_fire(&key, 1800).await { // 30 min cooldown
    ...
    pub async fn check_provision_failure(&self, bot_id: &str, attempt: u32) -> Option<AlertType> {
        if attempt >= self.config.provision_failure_threshold {
            ...
            if self.should_fire(&key, 300).await { // 5 min cooldown
                self.record_fired(key).await;
    ...
    pub async fn check_bot_offline(
        ...
        if let Some(last) = last_heartbeat {
            ...
            if elapsed.num_seconds() > self.config.offline_threshold_secs {
                ...
                if self.should_fire(&key, 900).await { // 15 min cooldown
                    self.record_fired(key).await;
    ...
    pub async fn fire_alert(&self, alert: &AlertType, severity: AlertSeverity) {
        let (title, message) = match alert {
            ...
            AlertType::BotOffline { bot_id, last_heartbeat } => {
                let last = last_heartbeat.map(|h| h.to_string())
...
pub fn spawn_offline_checker(pool: sqlx::PgPool, alert_manager: AlertManager) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            let bots = sqlx::query_as::<_, (uuid::Uuid, Option<chrono::DateTime<chrono::Utc>>, String)>(
            ...
            match bots {
                Ok(bots) => {
                    for (bot_id, last_hb, _status) in bots {
                        if let Some(alert) = alert_manager.check_bot_offline(
                            &bot_id.to_string(),
                            last_hb,
                        ).await {
                            alert_manager.fire_alert(&alert, AlertSeverity::Warning).await;
                        }
                    }
                }
                Err(e) => {
