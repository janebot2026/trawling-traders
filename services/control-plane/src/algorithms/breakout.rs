...
impl BreakoutAlgorithm {
    ...
    fn find_levels(&self, candles: &[Candle], period: usize) -> Option<(Decimal, Decimal)> {
        // (support, resistance)
        ...
        let resistance =
            highs.iter().rev().take(3).sum::<Decimal>() / Decimal::from(highs.len().min(3) as i64);
        let support =
            lows.iter().rev().take(3).sum::<Decimal>() / Decimal::from(lows.len().min(3) as i64);

        Some((support, resistance))
    }

    /// Calculate average volume
    fn avg_volume(&self, candles: &[Candle], period: usize) -> Decimal {
        let count = candles.len().min(period);
        if count == 0 {
            return Decimal::ZERO;
        }

        let sum: Decimal = candles.iter().rev().take(count).map(|c| c.volume).sum();

        sum / Decimal::from(count as i64)
    }

    /// Calculate price momentum (rate of change)
    fn momentum(&self, candles: &[Candle], period: usize) -> Decimal {
        if candles.len() < period {
            return Decimal::ZERO;
        }
...
impl Algorithm for BreakoutAlgorithm {
    ...
    fn generate_signal(&self, ctx: &MarketContext) -> Signal {
        ...
        let (support, resistance) = match self.find_levels(candles, self.params.lookback_period) {
            Some(levels) => levels,
            None => return Signal::hold(ctx.symbol.clone(), current_price, self.name.clone()),
        };
        ...
        let volume_threshold = self
            ...
            .map(|v| {
                Decimal::from_str(&v.to_string())
                    .unwrap_or_else(|_| Decimal::from(15) / Decimal::from(10))
            })
        ...
        if range == Decimal::ZERO {
            return Signal::hold(ctx.symbol.clone(), current_price, self.name.clone());
        }
        ...
        if current_price > resistance {
            ...
            if volume_spike {
                confidence = confidence * Decimal::from(12) / Decimal::from(10);
                // 1.2
            }

            // Boost with momentum
            if positive_momentum {
                confidence = confidence * Decimal::from(11) / Decimal::from(10);
                // 1.1
            }
        ...
        if position_in_range > Decimal::from(9) / Decimal::from(10) {
            // 0.9
            return Signal::hold(ctx.symbol.clone(), current_price, self.name.clone())
        ...
        if position_in_range < Decimal::from(1) / Decimal::from(10) {
            // 0.1
            return Signal::hold(ctx.symbol.clone(), current_price, self.name.clone())
        ...
        // No breakout - hold
        Signal::hold(ctx.symbol.clone(), current_price, self.name.clone())
    }
