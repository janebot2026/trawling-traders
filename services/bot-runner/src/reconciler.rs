...
impl HoldingsReconciler {
    ...
    pub async fn reconcile(
        ...
        info!("Starting holdings reconciliation for {}", self.wallet_address);
    ...
    async fn fetch_on_chain_holdings(
        &self,
    ) -> anyhow::Result<HashMap<String, u64>> {
        // Use claw-trader holdings command
        let result = self.executor.run_claw_trader(
            &[
                "holdings",
        ...
        let native_result = self.executor.run_claw_trader(
            &[
                "holdings-native",
                "--address", &self.wallet_address,
            ]
    ...
    fn compare_balances(
        ...
        for (mint, &amount) in on_chain {
            if !portfolio.positions.contains_key(mint) && amount > 0 {
                // Try to get symbol
                let symbol = crate::amount::get_token_info(mint)
                    .map(|t| t.symbol);
